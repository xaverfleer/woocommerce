	...
	() => {
		...
		test.beforeAll( async ( { browser, api } ) => {
			...
			await api
				...
				.then( ( response ) => {
					productIds = response.data.create.map(
						( item ) => item.id
					);
				} );
			...
			await api
				...
				.then( ( response ) => {
					orderIds = response.data.create.map(
						( order ) => order.id
					);
				} );
		...
		test( 'downloads revenue report as CSV', async ( { page } ) => {
			...
			try {
				...
				).toBeVisible( { timeout: 10000 } ); // fail fast if the snackbar doesn't display
				await expect(
					page.locator( '.components-snackbar' )
				).toHaveText( 'Your Revenue Report will be emailed to you.' );
			} catch ( e ) {
		...
		test( 'use filter by single product on products report', async ( {
			...
			await page
				.getByRole( 'button', { name: 'Single variation' } )
				.click();
			await page
		...
		test( 'analytics settings', async ( { page } ) => {
			...
			await page
				.getByRole( 'checkbox', { name: 'On hold' } )
				.first()
				.click();
			await page
